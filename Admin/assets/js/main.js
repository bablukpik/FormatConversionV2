var mapsData = {}, tmpMapsData = {}, timeOut, lineWeight = 0.5;
...
function initCanvas() {
    ...
    if (canvas) {
        ...
        if (window.innerWidth > bodyElm.outerWidth()) {
...
function handleCanvasClick(e) {
    ...
    for (var i = 0; i < connectors.length; i++) {
        ...
        if (c) {
            ...
            customDrawLine(pos1.left + eFrom.outerWidth() + 3,
                pos1.top + eFrom.outerHeight() / 2, pos2.left + 10, pos2.top + eTo.outerHeight() / 2, lineWeight);

            if (IsInPath(e)) {
                ctx.strokeStyle = 'red';
                ctx.stroke();
                btnRemoveCanvas.css('top', (e.clientY - 10) + offsetTop + 'px');
    ...
    if (ctx) {
        ...
        for (var i = 0; i < connectors.length; i++) {
            ...
            if (c) {
                ...
                customDrawLine(pos1.left + eFrom.outerWidth() / 2, pos1.top + eFrom.outerHeight() / 2,
                    pos2.left + 10, pos2.top + eTo.outerHeight() / 2, lineWeight);
            }
        }
    }
}
...
function compareData() {
    $("#fountainG").show();
    $.ajax({
        url: 'functions.php?action=compareData',
        type: 'post',
        data: mapsData,
...
function autoCompare() {
    var clientList, serverList, match = false;
...
function updateCounterSimilarWord(basicId, word)
{
    if (word && basicId) {
        $.ajax({
            url: 'functions.php?action=updateSimilarCounter',
            dataType: 'json',
            type: 'post',
